...
class MAHybridNetTrainer:
    ...
    def __init__(self, config: dict):
        ...
        self.output_root = Path(config['paths']['output_root']) / 'streak_removal'
        ...
        streak_root = config.get('paths', {}).get('streak_root', '')
        if streak_root and os.path.isdir(streak_root):
            self.logger.info(f"  Streak maps: {streak_root}")
        ...
        if self.use_wandb:
            try:
                ...
                wandb.init(
                    project="LD-CT-SR",
                    name=f"streak_removal_ep{self.start_epoch}",
                    config={
                        'epochs': self.epochs,
                        'batch_size': self.batch_size,
                        'lr': self.lr,
    ...
    def _load_fixed_sample(self):
        ...
        vis_cfg = self.config.get('streak_removal', {})
...
if __name__ == '__main__':
    ...
    if args.resume:
        if 'streak_removal' in cfg:
            cfg['streak_removal']['resume_ckpt'] = args.resume

    # 모드 결정 (CLI > config)
    mode = args.mode or cfg.get('mode', 'streak_removal')
    print(f"[Mode] {mode}")
