...
class CECTDataset(Dataset):
    ...
    def __init__(self, root, pairs_csv, split='train', image_size=512, num_slices=5):
        ...
        # Transform - resize만 필요 (정규화는 __getitem__에서 직접 처리)
        # Normalize 제거!
    ...
    def __getitem__(self, idx):
        ...
        # 데이터가 이미 [0, 1]로 정규화되어 있음 - 추가 정규화 불필요!
        # [-1, 1] 범위로만 변환
        ce_slice = ce_slice * 2.0 - 1.0  # [0, 1] → [-1, 1]
        
        # Tensor로 변환
        ce_tensor = torch.from_numpy(ce_slice).unsqueeze(0)  # [1, H, W]
        
        # Resize만 적용
        if ce_tensor.shape[1] != self.image_size or ce_tensor.shape[2] != self.image_size:
